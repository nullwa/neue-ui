    ...
    <div className={tm('flex items-center select-none', bordered && 'border border-secondary rounded-sm bg-secondary')}>
      <label htmlFor={`${label}-radio-item`} className={tm('w-full flex items-center gap-3 cursor-pointer py-3 px-4', direction === 'leading' ? 'flex-row' : 'flex-row-reverse', disabled && 'cursor-not-allowed opacity-60')}>
        <div className='relative flex items-center justify-center'>
          ...
          <span className={tm('absolute opacity-0 peer-checked:opacity-100 w-1.5 h-1.5 rounded-full bg-white', variant === 'default' && 'bg-gray-800')}/>
        </div>
        {label && (
...
const styles = cva('w-4.5 h-4.5 peer cursor-pointer transition-all appearance-none rounded-full shadow-xs border border-utility-gray-300 outline-none disabled:cursor-not-allowed disabled:text-fg-disabled disabled:bg-disabled disabled:shadow-xs disabled:ring-disabled_subtle',
  {
    variants: {
      variant: {
        brand: 'checked:border-brand-solid checked:bg-brand-solid checked:ring-2 checked:ring-brand/40',
        default: 'checked:border-gray-400 checked:bg-gray-300 checked:ring-2 checked:ring-gray-300/40',
        secondary: 'checked:border-gray-700 checked:bg-gray-600 checked:ring-2 checked:ring-gray-600/40',
