    ...
    <Primitive.Provider delay={delay}>
      <Primitive.Root>
        <Primitive.Trigger>
          <button
            type='button'
            className={tm('group relative flex h-max w-max cursor-pointer flex-col items-center gap-2 text-secondary outline-hidden transition duration-100 ease-linear')}>
            {children}
          </button>
        </Primitive.Trigger>
        <Primitive.Portal>
          <Primitive.Positioner side={placement} align='center' sideOffset={6}>
            <Primitive.Popup data-slot='tooltip-content' className={tm(
              'select-none z-50 flex max-w-2xl flex-col items-start rounded-md p-2 will-change-transform bg-secondary shadow-xs',
            ...
            {arrow && (
              <Primitive.Arrow
                ...
                <svg width='20' height='10' viewBox='0 0 20 10' fill='none'>
                  <path
                    className={'fill-bg-secondary border border-red-100'}
